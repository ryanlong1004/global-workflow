...
class State(object):
    ...
    def __eq__(self, arg):
        ...
        elif isinstance(arg, stubs.Node):
...
class Label(Attribute):
    ...
    def __init__(self, name, msg=""):
        self.load = stubs.Label(name, msg)
        CWN(self)
...
class Meter(Attribute):
    ...
    def __init__(self, name, beg, end, thr=None):
        if thr is None:
            thr = end
        self.load = stubs.Meter(name, beg, end, thr)
        CWN(self)
...
class Event(Attribute):
    ...
    def __init__(self, name="", num=0):
        ...
        if type(name) is int and type(num) == str:
            self.load = stubs.Event(name, num)
        elif type(name) is str and type(num) == int:
            self.load = stubs.Event(num, name)
        elif type(name) is str and type(num) == str:
            if num == "":
                self.load = stubs.Event(name)
            elif name == "":
                self.load = stubs.Event(num)
            else:
                self.load = stubs.Event(int(num), name)  # print("#str str")
        else:
            self.load = stubs.Event(name)
        CWN(self)
...
class Inlimit(Attribute):
    ...
    def __init__(self, fullpath, tokens=1):
        ...
        if USE_LIMIT:
            ...
            self.load = stubs.InLimit(name, path, int(tokens))
            self.path_ = path
            self.name_ = name
            self.tokens_ = tokens
            CWN(self)
...
class Trigger(Attribute):
    ...
    def __init__(self, expr, unk=False, anded=True):
        ...
        if type(expr) == Task:
            ...
            for index, name in enumerate(expr):
                ...
                if type(name) in (set, list, tuple):
                    ...
                    name = fullname
                elif type(name) in (stubs.Task, stubs.Family):
                    name = name.name()
                elif type(name) == str:
                    pass
                else:
        ...
        # self.load = "trigger"  # stubs.Trigger(self.expr)
        CWN(self)
        # NO_EXTERN_ALONE = 0 # set to 1 for test/activate
    ...
    def add_to(self, node):
        ...
        if node.load.get_trigger() is None:
            ...
            node.load.add_part_trigger(stubs.PartExpression(self.expr, True))
        return node
...
class Complete(Trigger):
    ...
    def __init__(self, expr, unk=False, anded=False):
        if sys.version_info.major == 2:
            ...
            super().__init__(expr, unk, anded)  # python3
...
class Clock(Attribute):
    ...
    def __init__(self, arg="24:00", hybrid=0):
        self.load = None
        if arg == "real":
            self.load = stubs.Clock(False)
        elif arg == "hybrid":
            self.load = stubs.Clock(True)
        elif type(arg) == str:
            ...
            self.load = stubs.Clock(arg, hybrid)
...
class Autocancel(Attribute):
    ...
    def __init__(self, arg):
        if type(arg) == str:
            ...
            self.load = stubs.Autocancel(arg)  # days
...
class Verify(Attribute):
    ...
    def __init__(self, arg):
        ...
        y, z = str(arg).split(":")
        kinds = {
            "aborted": stubs.State.aborted,
            "complete": stubs.State.complete,
            "active": stubs.State.active,
            "submitted": stubs.State.submitted,
            "unknown": stubs.State.unknown,
            "queued": stubs.State.queued,
        }
        self.load = stubs.Verify(kinds[y], int(z))
...
class Late(Attribute):
    ...
    def add_to(self, node):
        if USE_LATE and self.load is not None:
            if type(node) in (stubs.Family, stubs.Task):
...
class Edit(Attribute):
    ...
    def _set_tvar(self, key, val):
        ...
        if self.load is None:
            self.load = stubs.Variable(keyt, edit)
        else:
            next = self.next
...
class Repeat(Attribute):
    ...
    def __init__(self, name="YMD", start=20120101, end=21010101,
        ...
        if kind in ("date", ):
            ...
            if len(start) == 0:
                raise Exception
            self.load = stubs.RepeatString(name, start)
        elif "enum" in kind:
            if len(start) == 0:
                raise Exception
            self.load = stubs.RepeatEnumerated(name, start)
        elif kind == "day":
            self.load = stubs.RepeatDay(step)
        else:
            self.load = None
...
"""
    ...
    def __init__(self, arg="ecf:remove:3600"):
        ...
        types = stubs.ZombieType.names  # ecf user path
        kids = stubs.ChildCmdType.names  # complete label init abort event wait
        acts = stubs.ZombieUserActionType.names  # adopt remove kill fob fail block
        when = []
        if kid == "":
            pass
        elif "," in kid:
...
class Root(object):  # from where Suite and Node derive
    ...
    def add(self, item=None, *args):
        ...
        if not isinstance(self.load, stubs.Node):
...
def get_kind(item):
    if type(item) in (Defs, stubs.Defs):
        return "definition"
    if type(item) in (Suite, stubs.Suite):
        return "suite"
    if type(item) in (Family, stubs.Family):
        return "family"
...
def to_pyflow(node, container=None):
    ...
    if type(node) in (Alias, stubs.Alias):
        return
...
def shapes(node):
    if type(node) in (stubs.Task, Task):
        return "triangle"
    elif type(node) in (stubs.Family, Family):
        return "rect"
    elif type(node) in (stubs.Suite, Suite):
        return "invtriangle"
    return None
