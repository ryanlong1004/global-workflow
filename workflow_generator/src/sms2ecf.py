...
def translate(name, value=None):
    ...
    def add_ext(name, value):
        ...
        if name in ("ECF_JOB_CMD", "SMSCMD"):
            ...
            if "submit" in KILLCMD:
                ...
                rc = KILLCMD + " %s " % rc
        elif name in ("ECF_STATUS_CMD", "SMSSTATUS", "ECF_CHECK_CMD", "SMSJOBCHECK"):
    ...
    if ECF_MODE == "sms" and RAW_MODE:
        ...
        if name in ("ECF_JOB_CMD"):
            name = "SMSCMD"
        elif name in ("ECF_KILL_CMD"):
            name = "SMSKILL"
        elif name in ("ECF_STATUS_CMD"):
            name = "SMSSTATUS"
        value2 = value.replace("ECF_", "SMS")
        value = value2
        ...
        if name in ("ECF_JOB_CMD", "SMSCMD"):
            ...
            if "submit" in KILLCMD:
                ...
                value2 = KILLCMD + add_ext(value, value)
        elif name in ("ECF_STATUS_CMD", "SMSSTATUS", "ECF_CHECK_CMD", "SMSJOBCHECK"):
            if "submit" in STATCMD:
                value2 = STATCMD + add_ext(value, value) + " status"
            else:
                value2 = STATCMD + add_ext(value, value)
        return transl, value2
    return transl, value
...
def get(name, host="localhost", port=314159):
    ...
    user = os.getlogin()
    cmd = (
        "setenv -i HOME USER; set SMS_PROG %d; login %s %s 1; status > /dev/null ; get; show > %s; "
        % (port, host, user, name)
    )
